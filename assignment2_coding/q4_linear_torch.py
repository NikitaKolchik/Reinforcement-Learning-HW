...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        #print(torch.flatten(state, start_dim=1))
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        Q_samp = rewards + gamma * torch.max(target_q_values, dim=1)[0]
        Q_samp = torch.where(done_mask, rewards, Q_samp)

        # current q_values
        actions = F.one_hot(actions.to(torch.int64), num_actions) # shape = (batch_size, num_actions)
        Q_current = torch.sum(q_values * actions, dim=1)

        out = F.mse_loss(Q_current, Q_samp)
